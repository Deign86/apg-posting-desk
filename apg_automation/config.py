...
def validate_runtime_config(config: AppConfig, *, dry_run: bool) -> None:
    ...
    missing = []
    if config.ai.provider.lower() in {"nvidia", "nvidia-nim", "nim"}:
    ...
    return AppConfig(
        google_drive=GoogleDriveConfig(
            listings_folder_id=_value(
                data,
                "google_drive.listings_folder_id",
        ...
        processing=ProcessingConfig(
            ...
            concurrent_tasks=int(
                _value(data, "processing.concurrent_tasks", "CONCURRENT_TASKS", 3)
            ),
        ),
        ai=AIConfig(
